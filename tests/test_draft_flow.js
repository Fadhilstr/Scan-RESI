...
console.log('\nTEST 1: Generate Client Resi untuk RSS_EXPANDED & CODE_128...')
const clientResiRss = generateClientResi('RSS_EXPANDED')
const clientResiC128 = generateClientResi('CODE_128')

assert.strictEqual(typeof clientResiRss, 'string')
assert.strictEqual(clientResiRss.length, 8)
...
assert.strictEqual(
  LOCAL_PAKETS.some(p => p.nomor_resi === clientResiRss),
  false,
...
assert.strictEqual(saveRes.paket.nomor_resi, clientResiRss)
...
assert.strictEqual(scanSavedRes.resi, clientResiRss)
